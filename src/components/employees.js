...
function Employees() {
  ...
  const reload = async () => {
    try {
      ...
      const req = {
        url: 'http://localhost:4000/api/v1/adm/employee-view',
        method: 'get',
        headers: {
  ...
  const deleteItem = async (id) => {
    if (window.confirm('Are you sure you want to delete this item from the database?')) {
      var req = {
        url: `http://localhost:4000/api/v1/adm/employee-view?id=${id}`,
        method: 'delete',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('jwt')}`
        }
  ...
  const updateFormDataSave = async () => {
    try {
      const req = {
        url: `http://localhost:4000/api/v1/adm/employee-view?id=${updateFormDataState._id}`,
        method: 'post',
